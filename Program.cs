...
//1) показать должников у которых посты в домене rhyta.com или dayrep.com

var query1 = from debtor in debtors
             where debtor.Email[^10..] == "@rhyta.com" || debtor.Email[^11..] == "@dayrep.com"
             select debtor;

//2) показать должников у которых возраст от 36-и до 46-и

var query2 = from debtor in debtors
             where (DateTime.Now - debtor.BirthDay).Days >= 36 * 365 && (DateTime.Now - debtor.BirthDay).Days < 46 * 365
             select debtor;

//3) показать должников у которых долг не превышает 5000

var query3 = from debtor in debtors
             where debtor.Debt <= 5000
             select debtor;

foreach (var debtor in query)
{
